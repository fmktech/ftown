          ...
          <div className="flex items-center gap-2">
            <span
              style={{
                ...
                padding: "1px 4px",
                borderRadius: 3,
                background: session.shellType === "shell" ? "rgba(255, 170, 0, 0.12)" : session.shellType === "cursor" ? "rgba(168, 180, 255, 0.12)" : session.shellType === "codex" ? "rgba(16, 163, 127, 0.12)" : session.shellType === "zai" ? "rgba(100, 149, 237, 0.12)" : session.shellType === "kimi" ? "rgba(255, 105, 180, 0.12)" : session.shellType === "opencode" ? "rgba(186, 85, 211, 0.12)" : session.shellType === "deepseek" ? "rgba(0, 191, 255, 0.12)" : session.shellType === "grok" ? "rgba(255, 69, 96, 0.12)" : "rgba(0, 255, 136, 0.08)",
                color: session.shellType === "shell" ? "var(--status-pending)" : session.shellType === "cursor" ? "var(--shell-claude)" : session.shellType === "codex" ? "var(--shell-codex)" : session.shellType === "zai" ? "var(--shell-cursor)" : session.shellType === "kimi" ? "var(--shell-opencode)" : session.shellType === "opencode" ? "var(--shell-shell)" : session.shellType === "deepseek" ? "var(--shell-generic)" : session.shellType === "grok" ? "rgb(255, 69, 96)" : "var(--accent)",
                border: `1px solid ${session.shellType === "shell" ? "rgba(255, 170, 0, 0.2)" : session.shellType === "cursor" ? "rgba(168, 180, 255, 0.25)" : session.shellType === "codex" ? "rgba(16, 163, 127, 0.25)" : session.shellType === "zai" ? "rgba(100, 149, 237, 0.2)" : session.shellType === "kimi" ? "rgba(255, 105, 180, 0.2)" : session.shellType === "opencode" ? "rgba(186, 85, 211, 0.2)" : session.shellType === "deepseek" ? "rgba(0, 191, 255, 0.2)" : session.shellType === "grok" ? "rgba(255, 69, 96, 0.25)" : "rgba(0, 255, 136, 0.15)"}`,
                textTransform: "uppercase",
                fontFamily: "var(--font-mono)",
              }}
            >
              {session.shellType === "shell" ? "zsh" : session.shellType === "cursor" ? "cursor" : session.shellType === "codex" ? "codex" : session.shellType === "zai" ? "z.ai" : session.shellType === "kimi" ? "kimi" : session.shellType === "opencode" ? "opencode" : session.shellType === "deepseek" ? "deepseek" : session.shellType === "grok" ? "grok" : "claude"}
            </span>
